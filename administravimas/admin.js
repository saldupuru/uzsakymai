...
// Split out of index.html for performance - see loadScript() call at boot.
  ...
  function readyCombosSectionHtml(){
    ...
    const comboRows = combos.length
      ? combos.map((c,i)=>`<div class="wh-stock-row">
    ...
    return `<div class="inner-block wh-col" style="max-width:760px;">
      ...
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
        <button type="button" id="whComboColorSwatchBtn" title="Pasirinkti spalvą" style="width:36px;height:36px;border-radius:50%;cursor:pointer;padding:0;flex-shrink:0;font-size:16px;line-height:1;color:var(--choc-soft);${whComboDraftColor ? `background:${escapeAttr(whComboDraftColor)};border:2px solid var(--box-line);` : `background:#fff;border:2px dashed var(--box-line);`}">${whComboDraftColor ? '' : '+'}</button>
        <input type="color" id="whComboColorInput" value="${whComboDraftColor||'#cccccc'}" style="position:absolute;opacity:0;width:0;height:0;pointer-events:none;">

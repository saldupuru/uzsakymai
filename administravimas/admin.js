...
// Split out of index.html for performance - see loadScript() call at boot.
  ...
  function warehouseSectionHtml(){
    ...
    return `<div class="sub-section">
      ...
      ${readyCombosSectionHtml()}
      <div class="inner-block wh-col" style="max-width:760px;">
        ...
        <div class="wh-grid wh-grid-3">
          ...
          <div class="wh-col">
            <h4 style="font-size:12.5px;">Paruošti skoniai</h4>
            <div class="chip-list">${readyCombosUnavailableChipsHtml()}</div>
          </div>
        </div>
      </div>
  ...
  // Paruoštų derinių eilutės, kurios TURIMOS (rodomos „Paruošti skoniai" sąraše,
  // matomos ir kliento formoje) - indeksas i visada yra tikroji pozicija
  // warehouse.readyCombos masyve, nepriklausomai nuo filtravimo.
  function readyCombosAvailableRowsHtml(){
    const combos = warehouse.readyCombos || [];
    const avail = combos.map((c,i)=>({c,i})).filter(x=>!x.c.unavailable);
    return avail.length
      ? avail.map(({c,i})=>`<div class="wh-stock-row">
          <span class="chip-dot" style="background:${colorDisplayHex(c.color)}" title="${escapeAttr(c.color||'')}"></span>
          <span class="chip-name" style="flex:1;">${escapeHtml(c.flavor)}</span>
          <div style="display:flex;align-items:center;gap:4px;">
            <input type="number" min="0" step="1" class="flavor-stock-input" data-combo-stock="${i}" placeholder="likutis" value="${typeof c.stock==='number'?c.stock:0}">
            <span style="font-size:12px;color:var(--choc-soft);">g</span>
            <span class="chip-actions">
              <button class="chip-eye" data-combo-hide="${i}" title="Pažymėti kaip laikinai neturimą">👁️</button>
              <button class="chip-x" data-combo-remove="${i}" title="Ištrinti visam laikui">×</button>
            </span>
          </div>
        </div>`).join('')
      : `<p class="wh-empty">Nėra sukurtų derinių</p>`;
  }
  // Paruoštų derinių burbuliukai „Laikinai neturime" skiltyje - rodomi tik
  // pažymėti kaip laikinai neturimi, su mygtuku grąžinti juos atgal.
  function readyCombosUnavailableChipsHtml(){
    const combos = warehouse.readyCombos || [];
    const unavail = combos.map((c,i)=>({c,i})).filter(x=>x.c.unavailable);
    return unavail.length
      ? unavail.map(({c,i})=>`<span class="chip unavailable"><span class="chip-dot" style="background:${colorDisplayHex(c.color)}"></span><span class="chip-name">${escapeHtml(c.flavor)}</span><span class="chip-actions"><button class="chip-eye" data-combo-restore="${i}" title="Vėl turime">👁️</button></span></span>`).join('')
      : `<p class="wh-empty">Visi paruošti skoniai turimi</p>`;
  }
  function readyCombosSectionHtml(){
    const comboRows = readyCombosAvailableRowsHtml();

    return `<div class="inner-block wh-col" style="max-width:760px;">
      <h4>Paruošti skoniai</h4>
      <p class="combo-hint" style="margin:0 0 12px;">Susikurk iš anksto paruoštus skonio ir spalvos derinius su turimu kiekiu gramais. Įjungus perjungiklį, kliento užsakymo formoje bus galima rinktis tik iš šių paruoštų derinių (vietoj laisvo skonio ir spalvos pasirinkimo).</p>
      <div class="toggle-row" style="margin-bottom:14px;">
        <label class="toggle-switch">
          <input type="checkbox" id="whReadyCombosOn" ${warehouse.readyCombosFeatureOn?'checked':''}>
          <span class="toggle-track"></span>
        </label>
        <div>
          <div style="font-weight:700;font-size:14px;">Kliento formoje rodyti tik paruoštus derinius</div>
        </div>
      </div>
      <div class="wh-grid" style="margin-bottom:6px;">
        <div class="wh-stock-row wh-stock-row-narrow" style="border-bottom:none;padding-bottom:0;">
          <label for="whComboUsageLargeCake" style="font-size:12.5px;color:var(--choc-soft);">Gramų reikia dideliam tortui</label>
          <input type="number" min="0" step="1" id="whComboUsageLargeCake" placeholder="-" value="${(typeof warehouse.comboUsageLargeCake==='number')?warehouse.comboUsageLargeCake:''}">
        </div>
        <div class="wh-stock-row wh-stock-row-narrow" style="border-bottom:none;padding-bottom:0;">
          <label for="whComboUsageSmallCake" style="font-size:12.5px;color:var(--choc-soft);">Gramų reikia mažam tortui</label>
          <input type="number" min="0" step="1" id="whComboUsageSmallCake" placeholder="-" value="${(typeof warehouse.comboUsageSmallCake==='number')?warehouse.comboUsageSmallCake:''}">
        </div>
        <div class="wh-stock-row wh-stock-row-narrow" style="border-bottom:none;padding-bottom:0;">
          <label for="whComboUsageLargeJar" style="font-size:12.5px;color:var(--choc-soft);">Gramų reikia 1 dideliam indeliui</label>
          <input type="number" min="0" step="1" id="whComboUsageLargeJar" placeholder="-" value="${(typeof warehouse.comboUsageLargeJar==='number')?warehouse.comboUsageLargeJar:''}">
        </div>
        <div class="wh-stock-row wh-stock-row-narrow" style="border-bottom:none;padding-bottom:0;">
          <label for="whComboUsageSmallJar" style="font-size:12.5px;color:var(--choc-soft);">Gramų reikia 1 mažam indeliui</label>
          <input type="number" min="0" step="1" id="whComboUsageSmallJar" placeholder="-" value="${(typeof warehouse.comboUsageSmallJar==='number')?warehouse.comboUsageSmallJar:''}">
        </div>
      </div>
      <div class="wh-stock-row wh-stock-row-narrow" style="border-bottom:none;padding-bottom:0;">
        <label for="whMinComboStock" style="font-size:12.5px;color:var(--choc-soft);">Mažiausias leistinas likutis (g)</label>
        <input type="number" min="0" step="1" id="whMinComboStock" placeholder="-" value="${(warehouse.minComboStock===null||warehouse.minComboStock===undefined)?'':warehouse.minComboStock}">
      </div>
      <div class="checkrow" style="margin:2px 0 12px;">
        <input type="checkbox" id="whMinComboStockAlertsOn" ${warehouse.minComboStockAlertsOn!==false?'checked':''}>
        <label for="whMinComboStockAlertsOn" style="margin:0;text-transform:none;font-weight:400;font-size:12.5px;color:var(--choc-soft);">Rodyti pranešimus apie mažus likučius virš kalendoriaus</label>
      </div>
      <p class="pick-label" style="margin-bottom:4px;">Spalva</p>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
        <span style="position:relative;display:inline-flex;width:36px;height:36px;flex-shrink:0;">
          <label for="whComboColorInput" id="whComboColorSwatchBtn" title="Pasirinkti spalvą" style="position:absolute;inset:0;display:inline-flex;align-items:center;justify-content:center;border-radius:50%;cursor:pointer;font-size:16px;line-height:1;color:var(--choc-soft);${whComboDraftColor ? `background:${escapeAttr(whComboDraftColor)};border:2px solid var(--box-line);` : `background:#fff;border:2px dashed var(--box-line);`}">${whComboDraftColor ? '' : '+'}</label>
          <input type="color" id="whComboColorInput" value="${whComboDraftColor||'#cccccc'}" style="position:absolute;inset:0;width:36px;height:36px;opacity:0;cursor:pointer;border:none;padding:0;margin:0;">
        </span>
        <span class="combo-hint" style="margin:0;">${whComboDraftColor ? escapeHtml(whComboDraftColor) : 'Pasirink spalvą paspaudęs burbuliuką'}</span>
      </div>
      <div class="wh-add-row" style="flex-wrap:wrap;margin-top:8px;">
        <input type="text" id="whComboFlavorInput" placeholder="Skonis">
        <input type="number" min="0" step="1" id="whComboStockInput" placeholder="Kiekis (g)" style="max-width:110px;">
        <button class="btn btn-primary btn-sm" id="whAddCombo">+ Pridėti</button>
      </div>
      <div class="flavor-stock-list">${comboRows}</div>
    </div>`;
  }
  ...
  function orderProductSummaryPlain(arr, singular, plural){
    ...
    return arr.map((unit,i)=>{
      ...
      const txt = combos.length ? combos.map(c=>`${colorDisplayName(c.color)} - ${c.flavor||'?'}`).join(' | ') : 'nepasirinkta';
  ...
  function buildOrdersExportRows(list){
    return list.map(o=>{
      const jarLargeTxt = (o.jarLargeCombos||[]).filter(c=>c.flavor||c.color).map(c=>`${colorDisplayName(c.color)} - ${c.flavor||'?'}`).join(' | ');
      const jarSmallTxt = (o.jarSmallCombos||[]).filter(c=>c.flavor||c.color).map(c=>`${colorDisplayName(c.color)} - ${c.flavor||'?'}`).join(' | ');
  ...
  async function exportIncomeExpenseJournalToExcel(){
    ...
    const entries = [
      ...invoices.filter(inv=>inv.incomeRecognized && inv.issueDate && inv.issueDate>=start && inv.issueDate<=end)
        .map(inv=>({
  ...
  function journalSectionHtml(){
    ...
    let journalEntries = [
      ...invoices.filter(inv=>inv.incomeRecognized && inv.issueDate && inv.issueDate>=journalStart && inv.issueDate<=journalEnd).map(inv=>({type:'income', date:inv.issueDate, label:`SF ${formatInvoiceNumber(inv)} - ${inv.buyerName||'-'}`, amount:inv.totalAmount, ref:inv})),
  ...
  function reminderBannerHtml(){
    ...
    lowStockCombos().forEach(({flavor,color,stock})=>items.push({icon:'🧁', text:`${escapeHtml(color)} - ${escapeHtml(flavor)} - mažas paruošto derinio likutis: ${stock} g - reikia paruošti daugiau`, cls:'rem-warn', filter:null, gotoWarehouse:true}));
  ...
  function computeFinancialReport(start, end){
    const incomeEntries = invoices.filter(inv=>inv.incomeRecognized && inv.issueDate >= start && inv.issueDate <= end);

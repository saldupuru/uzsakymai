  ...
  function readyCombosSectionHtml(){
    ...
    const comboRows = combos.length
      ? combos.map((c,i)=>`<div class="wh-stock-row">
          <span class="chip-dot" style="background:${colorNameToHex(c.color)}"></span>
          <span class="chip-name" style="flex:1;">${escapeHtml(c.color)} - ${escapeHtml(c.flavor)}</span>
          <div style="display:flex;align-items:center;gap:4px;">
    ...
    return `<div class="inner-block wh-col" style="max-width:760px;">
      ...
      ${warehouse.colors.length ? `<div class="color-bubbles" id="whComboColorPicker">
        ${warehouse.colors.map(c=>`<button type="button" class="color-bubble ${whComboDraftColor===c?'selected':''}" style="background:${colorNameToHex(c)}" data-value="${escapeAttr(c)}" title="${escapeAttr(c)}"></button>`).join('')}
      </div>` : `<p class="combo-hint">Pirmiausia įvesk bent vieną spalvą skiltyje „Turimos spalvos" aukščiau.</p>`}

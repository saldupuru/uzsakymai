...
// Split out of index.html for performance - see loadScript() call at boot.
  ...
  // Photo upload/remove UI + description/price fields for one product type
  // (cakes or jars) inside the Produktai tab - mirrors the Puslapis tab's
  // "Nuotraukų galerija" block (pageSectionHtml above), just scoped to
  // orderSettings.productsCakes / orderSettings.productsJars instead of the
  // shared landingGalleryPhotos list, and with its own description/price
  // fields since each product type's detail page needs its own text.
  function productTypeContent(type){
    const key = type==='cakes' ? 'productsCakes' : 'productsJars';
    const data = orderSettings[key] || {photos:[], description:'', priceMin:'', priceMax:''};
    ...
    return `
        ...
        <div style="display:flex;gap:10px;margin-top:10px;">
          <div class="field" style="flex:1;"><label>Kaina nuo (€)</label><input type="number" step="0.01" min="0" id="setProduct${prefix}PriceMin" value="${escapeAttr(data.priceMin||'')}" placeholder="Pvz. 20"></div>
          <div class="field" style="flex:1;"><label>Kaina iki (€)</label><input type="number" step="0.01" min="0" id="setProduct${prefix}PriceMax" value="${escapeAttr(data.priceMax||'')}" placeholder="Pvz. 150"></div>
        </div>`;
  }

  // "Produktai" admin tab - two collapsible parts (Tortai / Indeliai), each
  // feeding its own /produktai/cukraus-vatos-tortai or
  // /produktai/cukraus-vatos-indeliai public page, plus the shared section
  // heading shown above the two cards on the main landing page.
  function productsSectionHtml(){
    ...
    return `<div class="sub-section">
      <p class="combo-hint" style="margin-bottom:14px;max-width:560px;">Ši skiltis rodoma pagrindiniame puslapyje, prieš dažniausiai užduodamus klausimus - dvi kortelės (Tortai, Indeliai), kurios nuveda į atskirus, atskirai indeksuojamus produktų puslapius.</p>
      ${landingPageCollapsibleHtml('productsIntro', 'Skilties antraštė', introContent)}
      ...
      <div class="inner-block" style="max-width:560px;">
        <button class="btn btn-primary btn-sm" id="saveProductsSectionBtn">Išsaugoti</button>
      </div>
    </div>`;
  }
